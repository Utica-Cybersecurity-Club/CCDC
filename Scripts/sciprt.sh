...
# Function to allow for user management
user_management() {
    log "Manageing Users ..."
    # Prompt for passwords
    read -p "Enter password for the admin account: " ADMIN_PASS
    read -p "Enter password for the root account: " ROOT_PASS
    read -p "Enter password for new sudoers account: " SUDO_PASS

    # Change passwords
    echo "$(logname):$ADMIN_PASS" | chpasswd
    echo "root:$ROOT_PASS" | chpasswd

    # Create a sudoers account that cannot log in
    SUDO_USER="sudo_nologin"
    if ! id "$SUDO_USER" &>/dev/null; then
        useradd -M -s /usr/sbin/nologin "$SUDO_USER"
        echo "$SUDO_USER:$SUDO_PASS" | chpasswd
        usermod -aG sudo "$SUDO_USER"
        echo "$SUDO_USER created with sudo privileges but no login access."
    else
        echo "$SUDO_USER already exists, skipping creation."
    fi

    BASE_USERS=("root" "$(logname)" "$SUDO_USER")  # Keep only root, the main user, and the sudo non-login user

    # Remove all non-base users
    echo "Removing all non defualt users"
    ...
    if [[ -f "$USER_FILE" ]]; then
        while IFS="::" read -r username groups locked; do
            if id "$username" &>/dev/null; then
                ...
                if [[ "$locked" == "yes" ]]; then
                    passwd -l "$username"
                    echo "Added locked user: $username with groups: $groups"
                fi
                echo "Added user: $username with groups: $groups"
            fi
        done < "$USER_FILE"
    fi

    # Restricitng all non exempt users
    read -p "List out users to not restrict" EXEMPT_USERS
    
    echo "Restricitng users..."

    for user in $(awk -F: '$3 >= 1000 && $7 !~ /nologin|false/ {print $1}' /etc/passwd); do
        if [[ ! " ${EXEMPT_USERS[@]} " =~ " $user " ]]; then
            echo "Setting restricted shell for $user"
            chsh -s /bin/rbash "$user"
        else
            echo "Exempted user: $user"
        fi
    done
...
setup_firewall() {
    ...
    install_package "ufw"
    sudo ufw reset || handle_error "Failed to reset UFW"
...
# Function to setup Suricata
setup_suricata() {
    log "Installing and Configuring Suricata..."
    install_package "suricata"
    suricata-update update-sources || handle_error "Failed to update Suricata"
    suricata-update || handle_error "Failed to update Suricata"
    systemctl restart suricata || handle_error "Failed to start Suricata service"
    systemctl enable suricata || handle_error "Failed to start Suricata service"
